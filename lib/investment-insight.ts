...
function buildSystemInstruction(locale: z.infer<typeof localeSchema>): string {
  ...
  return [
    ...
    '가독성: 딱딱한 설명문보다 스캔하기 쉽게 쓰세요. 핵심 종목명·비중·리스크 키워드는 **볼드**로 감싸세요 (예: **테슬라** 비중이 높아요).',
    '문장/불릿 앞에 이모지를 1개만 가볍게 쓸 수 있습니다. 문장마다·과도하게 쓰지 마세요. 예: 📌 **집중도**, ⚖️ **균형**, ⚠️ **리스크**.',
    ...
    'summary: 장기 보유 관점에서 포트폴리오가 균형 잡혔는지 2~4문장. 핵심은 **볼드**.',
    'outlook: 보유 구조에 대한 중장기 관점 코멘트 2~4문장 (당일 시황 나열 금지).',
    'suggestions: 실행 가능한 마인드/프로세스 팁 2~4개 (종목 매매 지시 금지). 각 항목 앞에 이모지 1개 권장.',
    'risks: 주의할 점 2~4개. 각 항목 앞에 ⚠️ 등 이모지 1개 권장.',
  ].join('\n');
}
