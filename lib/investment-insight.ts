...
function buildSystemInstruction(locale: z.infer<typeof localeSchema>): string {
  ...
  return [
    ...
    '문장/불릿 앞에 이모지를 최대 1개만 쓰세요. 이모지만 나열하지 마세요. 예: 📌 **집중도**, ⚖️ **균형**, ⚠️ **리스크**.',
    ...
    'suggestions: 실행 가능한 마인드/프로세스 팁 2~4개 (종목 매매 지시 금지). 앞에 이모지 1개 + 핵심 **볼드**.',
    'risks: 주의할 점 2~4개. 앞에 ⚠️ 등 이모지 1개 + 핵심 **볼드**.',
  ].join('\n');
}
