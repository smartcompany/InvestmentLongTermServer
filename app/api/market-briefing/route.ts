...
import {
  ...
  marketBriefingRequestSchema,
} from '@/lib/market-briefing';

function clientErrorMessage(error: unknown): string {
  if (error instanceof z.ZodError) {
    return 'Invalid market briefing response from AI';
  }
...
export async function POST(request: NextRequest) {
  let body: z.infer<typeof marketBriefingRequestSchema>;
  try {
    body = marketBriefingRequestSchema.parse(await request.json());
  } catch (error) {
    if (error instanceof z.ZodError) {
  ...
  try {
    const briefing = await generateMarketBriefing(body);
    return NextResponse.json(briefing);
  } catch (error) {
    const message = clientErrorMessage(error);
    console.error('[POST /api/market-briefing] error:', message);
    return NextResponse.json(
      {
        error: message,
